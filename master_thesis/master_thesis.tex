...
The research problem addressed in this thesis is: \emph{Is it possible to achieve accurate medical image classification with a single optimally chosen convolution kernel?}
This framing contrasts with standard CNN practice, where many generic kernels are learned jointly, and asks whether one carefully optimized kernel can provide a compact and interpretable alternative.
...
The literature points to three persistent needs: data-efficient learning, robust priors, and interpretable decision mechanisms \citep{litjens2017survey,pachetti2024fewshot,rudin2019blackbox,salahuddin2022transparency}. This thesis positions itself at the intersection of those needs. Unlike many current pipelines, it does not seek to replace all manual structure with a deeper network. Instead, it asks how much performance can be obtained from a carefully adapted kernel representation, and how the resulting model behavior changes when one varies bank diversity, selection strategy, parameter sampling, rotational structure, and radial compactness.
...
The complete project implementation used throughout the thesis (data preparation, kernel models, experiment scripts, and figures) is available at \href{https://github.com/Masoudvahid/Kernel-Based-Cancer-Classification}{GitHub: Masoudvahid/Kernel-Based-Cancer-Classification}.
...
Two main selection mechanisms are used in the project: MMR and ABESS \citep{goldstein1998mmr,zhu2020abess,zhu2022abess}. In the MMR variant, kernel relevance is balanced against redundancy. If $S$ is the set of already selected kernels and $j$ is a candidate, the selection score has the familiar form
\[
...
This reduces the number of explicit features and can yield visually meaningful aggregate filters. However, the thesis results show that pre-classification aggregation introduces a trade-off. When per-kernel responses are first collapsed through a nonlinear statistic such as absolute maximum, the operation ``compose then respond'' is not equivalent to ``respond then classify''. As a result, some discriminative information is lost. The composition stage is therefore useful as an efficiency and interpretability mechanism, but it is not automatically performance-preserving.

\section{Adaptive Sampling of Kernel Parameters}
A third adaptive mechanism concerns how kernel parameters are sampled. Random sampling is the legacy baseline, but the structured code also supports Sobol quasi-Monte Carlo sampling and Latin hypercube sampling \citep{mckay1979lhs}. The thesis focuses on Sobol sampling because low-discrepancy sequences cover bounded parameter spaces more uniformly than naive random draws \citep{sobol1967cube}. In a finite kernel-bank regime, this matters: better coverage of parameter space can improve the odds of discovering discriminative filters without increasing bank size excessively.
...
The final and most important adaptation re-parameterizes the kernel itself into a radial form. Instead of learning or selecting many free 2D filters, the model defines a kernel by ring-shaped basis functions $B_r$ and shell coefficients $f_r$:
\[
...
The results are best read as a chronological development path. The project begins with sparse bank selection, then tests stronger aggregation and search strategies, then studies rotational structure, and finally moves into progressively more structured radial models. Not every stage uses exactly the same split, pooling rule, or optimization objective, so the chapter distinguishes between two kinds of comparison:
\begin{itemize}
...
Two points are immediately visible. First, the best AUC does not come from the largest selected set. A small MMR subset of three kernels already reaches a test AUC of 0.9239 which nicely supports our more general hypothesis of small models being sufficient for good classification. Second, increasing feature count shifts the operating point: the 300-kernel MMR model reduces AUC but increases thresholded accuracy relative to the smaller sparse models. This suggests that adaptive selection is not only a relevance filter but also a mechanism for controlling the complexity-versus-stability trade-off of the final classifier.
...
In the right column of Figure~\ref{fig:exp019heatmaps}, some red activations likely come from the image's right border and should be treated as an artifacts rather than lesion evidence. That ambiguity is exactly why the thesis emphasizes interpretable feature design and qualitative review together.
...
This directly supports the working hypothesis that small structured models can remain sufficient for useful classification in this dataset.
...
\begin{figure}[H]
    ...
    \caption{Heatmap summaries for two reduced-bank sparse variants. Even very small kernel banks can remain qualitatively informative, though their AUC trails the strongest larger-bank models. Border-only activation near the image edge in some right-column examples is a rendering artifact and is not interpreted as lesion evidence.}
    \label{fig:reducedbankheatmaps}
\end{figure}

\section{Composition and QMC Sampling}
After sparse selection had been validated, the next step was to ask whether many useful kernels could be aggregated into a smaller set of composite filters. Composition improves compactness and visual readability, but the aggregation happens before classification, so part of the discriminative signal is lost. In the strongest composition setting used here, the composite model reaches test AUC 0.8933 and test accuracy 0.8150.

What remains consistent across the composition studies is the qualitative conclusion: composition can produce compact and visually meaningful filters, but it often falls short of the non-aggregated selected-kernel classifier because nonlinear response pooling makes pre-classification aggregation lossy.
...
\begin{figure}[H]
    ...
    \caption{Composite kernel grid from the rotation/composition experiments. Composition improves compactness and visual interpretability, but usually aggregates away some discriminative information.}
    \label{fig:compositegrid}
\end{figure}

The next improvement targeted search rather than pre-classification aggregation. Replacing random parameter draws with Sobol quasi-Monte Carlo sampling gives the cleanest matched gain in the thesis because it changes only how the kernel space is explored. Table~\ref{tab:qmc} summarizes the key comparison.
...
Compared with the final signed learnable-radius model (test AUC 0.9101), this averaging-like constrained variant (test AUC 0.8885) confirms that brightness-dominated behavior alone is not sufficient for the best discrimination.
...
\begin{figure}[H]
    ...
    \caption{Geometric ablation versus radial heatmap comparison. The geometric model spreads probability broadly across boundaries and texture, whereas the radial model is more selective around the suspicious region. Occasional right-edge red bands are border artifacts from patch scanning and are not treated as lesion localization.}
...
Second, visually different kernels can still produce similar classification behavior when the response map is reduced to one or a few scalar values. The later signed top-$k$ formulation was introduced partly to reduce that ambiguity and make the score depend on a more consistent signed pattern rather than on one dominant absolute peak. In the final learned profile, negative shell values are essential because they explicitly reject pure averaging behavior, while the outer shells fading toward zero indicate finite effective support rather than unconstrained long-range smoothing.
...
\begin{figure}[H]
    ...
    \caption{Final learnable-radius radial model. The summary shows the signed normalized profile, the learned support gate, and the resulting kernel. The negative shell values are important because they show that the kernel is not a simple averaging filter, and the outer values fading toward zero indicate finite effective support.}
    \label{fig:radialsummary}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.48\linewidth]{../Reports/figures/radial_single_kernel_original_vs_heatmap/IMG015_original_vs_heatmap.png}\hfill
    \includegraphics[width=0.48\linewidth]{../Reports/figures/radial_single_kernel_original_vs_heatmap/IMG033_original_vs_heatmap.png}\par\smallskip
    \includegraphics[width=0.48\linewidth]{../Reports/figures/radial_single_kernel_original_vs_heatmap/IMG263_original_vs_heatmap.png}\hfill
    \includegraphics[width=0.48\linewidth]{../Reports/figures/radial_single_kernel_original_vs_heatmap/IMG351_original_vs_heatmap.png}
    \caption{Single-kernel radial heatmap summary for four cases: two malignant (IMG015, IMG033) and two normal (IMG263, IMG351). Each panel shows original image (left) and heatmap (right), with probability color bar from 0 (blue) to 1 (red).}
    \label{fig:radialheatmap}
\end{figure}

\section{Comparison with a Deep CNN Baseline}
To address the reviewer request for explicit deep-learning comparison, the thesis includes a compact patch-CNN baseline trained on the same train/validation/test split used in the main kernel experiments. The CNN is intentionally small (four convolutional blocks with batch normalization \citep{ioffe2015batchnorm}, max pooling, dropout \citep{srivastava2014dropout}, and a linear head) so that the comparison remains fair to low-data constraints rather than relying on a very large pretrained architecture.

Table~\ref{tab:cnncomparison} summarizes the core quantitative comparison. Three points are clear. First, the averaging-like constrained radial variant is weaker than the signed variants. Second, the final signed radial model remains competitive while using a single interpretable kernel. Third, the CNN gives the strongest absolute AUC/accuracy on this split.

\begin{table}[H]
\centering
\caption{Kernel-based models versus deep CNN baseline on the shared held-out split.}
\label{tab:cnncomparison}
\begin{tabular}{lcc}
\toprule
Model & Test AUC & Test ACC \\
\midrule
Averaging-like constrained radial variant & 0.8885 & 0.8118 \\
Final signed radial with learnable radius & 0.9101 & 0.8449 \\
Best selected kernel bank (Sobol-QMC, 90 kernels) & 0.9387 & 0.8866 \\
Deep patch CNN baseline & \textbf{0.9626} & \textbf{0.9021} \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:cnndetailed} and Table~\ref{tab:cnnconfusion} report detailed CNN operating characteristics on the test split.

\begin{table}[H]
\centering
\caption{Detailed deep patch CNN test metrics (threshold 0.5).}
\label{tab:cnndetailed}
\begin{tabular}{lc}
\toprule
Metric & Value \\
\midrule
...
\begin{table}[H]
\centering
\caption{Deep patch CNN test confusion matrix (threshold 0.5).}
\label{tab:cnnconfusion}
\begin{tabular}{lcc}
\toprule
...
Figure~\ref{fig:cnnrocs} compares held-out ROC behavior for the strongest selected-kernel classifier and the CNN. Figure~\ref{fig:cnnqualitative} compares qualitative heatmaps on a representative test image; the CNN output is sharper in this example, but both methods focus on similar suspicious tissue regions. Additional right-edge activation in some CNN overlays is a border artifact from sliding-window inference and is ignored in interpretation.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.48\linewidth]{../publication/figures/selected_kernel_roc_pr.png}\hfill
    \includegraphics[width=0.48\linewidth]{../publication/figures/cnn_roc_curve.png}
    \caption{Held-out ROC comparisons: selected-kernel model (left) and deep patch CNN (right).}
    \label{fig:cnnrocs}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.48\linewidth]{../publication/figures/selected_kernel_heatmap_example.png}\hfill
    \includegraphics[width=0.48\linewidth]{../publication/figures/cnn_heatmap_example.png}
    \caption{Qualitative heatmap comparison on a representative test image: selected-kernel model (left) and deep patch CNN (right).}
    \label{fig:cnnqualitative}
\end{figure}
