...
The notebook code is now factored into a reusable package:

- `kernel_model/` modules for data, kernels, responses, selection, models, plots, pipeline, and configs
- CLI entrypoint: `python kernel_model.py --help`
- Notebook that imports the package: `kernel_model_structured.ipynb`
- Optional selector: `--selection-method abess` (requires `pip install abess`); otherwise defaults to MMR.
- Optional subsampling: `--train-subset-frac` or `--train-subset-size` to use fewer samples for faster runs.
- Outputs are auto-versioned under `results/exp_###` (or your `--out-dir` base).
...
- matplotlib
- tqdm
- abess (optional; required only if using `--selection-method abess`)
